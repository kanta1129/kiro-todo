...
const mockCalculateFreshness = vi.fn();
...
describe('Task Lifecycle Integration Tests', () => {
  beforeEach(() => {
    ...
    mockCalculateFreshness.mockReturnValue({
      state: '新規',
      daysUntilDue: 5,
      adjustedDays: 5,
      shouldUpdate: false,
  ...
  describe('Freshness State Transitions Over Time', () => {
    it('should transition through all freshness states based on time', () => {
      ...
      // Test transition to '期限接近' (approaching deadline)
      mockCalculateFreshness.mockReturnValue({
        state: '期限接近',
        daysUntilDue: 2,
        adjustedDays: 2,
      ...
      // Test transition to '期限間近' (critical deadline)
      mockCalculateFreshness.mockReturnValue({
        state: '期限間近',
        daysUntilDue: 0.5,
        adjustedDays: 0.5,
      ...
      // Test transition to '期限切れ' (overdue)
      mockCalculateFreshness.mockReturnValue({
        state: '期限切れ',
        daysUntilDue: -1,
        adjustedDays: -1,
    ...
    it('should handle priority-based freshness decay rates', () => {
      ...
      // Create high priority task (should decay faster)
      store.addTask({
      ...
      // Create low priority task (should decay slower)
      store.addTask({
      ...
      mockCalculateFreshness.mockImplementation((task: Task) => {
  ...
  describe('Tombstone Functionality', () => {
    it('should allow task revival by updating due date', () => {
      ...
      // Revive task by updating due date to future
    ...
    it('should maintain tombstone tasks in separate visual section', () => {
      ...
      // Mock freshness states
      mockCalculateFreshness.mockImplementation((task: Task) => {
        if (task.title === 'Fresh Task') {
          return { state: '新規', daysUntilDue: 30, adjustedDays: 30, shouldUpdate: true };
        } else {
          return { state: '期限切れ', daysUntilDue: -30, adjustedDays: -30, shouldUpdate: true };
        }
      });
